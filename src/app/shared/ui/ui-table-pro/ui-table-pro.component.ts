...
export class UiTableProComponent<T = any> {
  ...
  totalPages = computed(() =>
      Math.ceil((this.total() || 0) / this.pageSize())
  );
  from = computed(() =>
  ...
  loadStoresFromStorage() {
    try {
      ...
      if (this.stores().length) {
        const isValid =
            selectedId && this.stores().some((s) => s.id === selectedId);
  ...
  onSort(c: UiTableColumn<T>) {
    ...
    if (currKey !== key) {
      ...
      const next =
          currDir === 'asc' ? 'desc' : currDir === 'desc' ? '' : 'asc';
  ...
  toggleColumn(c: UiTableColumn<T>) {
    ...
    const next = cols.map((x) =>
        this.keyToString(x.key) === this.keyToString(c.key)
            ? { ...x, hidden: !x.hidden }
            : x
    );
